...
def push_bottle():
    ...
    i = 0
    while not robot_moves.object_near(100):  # Spin in small angle increments until doing a 360.
        robot_moves.spin_right(angle=4, rev=True)  # Could be smaller?
    while robot_moves.object_near():  # Spin in small angle increments until doing a 360.
        robot_moves.spin_right(angle=4, rev=True)  # Could be smaller?
        i += 1
